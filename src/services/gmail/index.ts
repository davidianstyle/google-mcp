...
export function registerGmailTools(server: McpServer, ctx: ServiceContext): void {
  ...
  server.tool("gmail_send_email", "Send a brand-new email. To reply within an existing thread (correct threading headers, Re: subject), use gmail_reply_to_email instead.", {
    ...
    body: z.string().describe("Email body (plain text). IMPORTANT: do NOT hard-wrap lines — write each paragraph as one long line; separate paragraphs with a blank line. Lines starting with \"- \" become bullets and \"1. \" become numbered items. An HTML part is generated automatically so Gmail keeps paragraphs flowing; only pass htmlBody if you need custom markup."),
    htmlBody: z.string().optional().describe("HTML version of the email body"),
    cc: z.array(z.string()).optional().describe("CC recipients"),
    bcc: z.array(z.string()).optional().describe("BCC recipients"),
    mimeType: z.enum(["text/plain", "text/html", "multipart/alternative"]).optional().default("multipart/alternative").describe("Defaults to multipart/alternative (plain text + auto-generated HTML). text/plain forces a text-only message, which Gmail hard-wraps at ~70 columns when the draft is opened."),
  ...
  server.tool("gmail_reply_to_email", "Reply to an existing email, correctly threaded. Pass the Gmail API message id of the message you're replying to; this fetches its RFC822 Message-ID/References/Subject and builds a proper In-Reply-To + References chain, a 'Re:' subject, and reuses the thread. Use this instead of gmail_send_email whenever you're responding to a received message.", {
    ...
    body: z.string().describe("Reply body (plain text). IMPORTANT: do NOT hard-wrap lines — write each paragraph as one long line; separate paragraphs with a blank line. Lines starting with \"- \" become bullets and \"1. \" become numbered items. An HTML part is generated automatically so Gmail keeps paragraphs flowing; only pass htmlBody if you need custom markup."),
